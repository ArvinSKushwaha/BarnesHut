...
use barnes_hut::octtree::Octree as OctreeNode;
    ...
    for size in (4usize..=20usize).map(|v| 1usize << v) {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                points.iter_mut().map(Vec3::as_mut_slice).for_each(|s| {
                    s.copy_from_slice(&rand::thread_rng().sample::<[f32; 3], Standard>(Standard))
                });
                ...
                b.iter(|| OctreeNode::construct(black_box(&points), black_box(&masses)));
            },
        );
    }
